...
const fallback={
 ...
 offers:(Array.isArray(window.SALLA_EXCEL_OFFERS)&&window.SALLA_EXCEL_OFFERS.length?window.SALLA_EXCEL_OFFERS:[]),insurers:[
...
function normalizePayload(j){if(!j||typeof j!=="object")return null;if(j.data&&typeof j.data==="object")j=j.data;const out={...structuredClone(fallback),settings:{...fallback.settings,...(j.settings||{})}};['stats','socials','services','doctors','insurers','suppliers'].forEach(k=>{if(Array.isArray(j[k])&&j[k].length)out[k]=j[k]});const bundledExcel=Array.isArray(window.SALLA_EXCEL_OFFERS)&&window.SALLA_EXCEL_OFFERS.length;if(Array.isArray(j.offers)&&j.offers.length&&(!bundledExcel||String(j.settings?.offersSource||'')==='excel-import'))out.offers=j.offers;if(Array.isArray(out.stats)){out.stats=out.stats.map(x=>{const ar=String(x.labelAr||''),en=String(x.labelEn||'');if((Number(x.value)===24||ar.includes('24 تخصص')||en.includes('24 specialties'))&&String(x.icon||'').includes('✚'))return {...x,value:11,labelAr:'أكثر من 11 تخصص',labelEn:'More than 11 specialties'};return x})}return out}
function readCachedData(){
  try{
    const raw=localStorage.getItem('alsiteen_content_cache_v2');
    if(!raw)return null;
...
function writeCachedData(payload){
  try{localStorage.setItem('alsiteen_content_cache_v2',JSON.stringify({savedAt:Date.now(),payload}))}catch(e){}
}
